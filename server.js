...
<body>
<header><div><h1>Радио-Т</h1><div class="sub">неофициальный веб-плеер</div></div><a href="https://radio-t.com/" target="_blank" style="color:#ff7a18">radio-t.com ↗</a></header>
<main class="wrap">
...
function render(items){list.innerHTML=items.map((e,i)=>`<article class="episode"><h2>${esc(e.title||("Выпуск "+(e.num||"")))}</h2><div class="meta">${esc(date(e.date))}</div><div class="desc">${esc((e.body||e.description||"").replace(/<[^>]+>/g,"").slice(0,300))}</div><p><button class="btn" data-i="${i}">▶ Слушать</button></p></article>`).join("");[...list.querySelectorAll("[data-i]")].forEach(b=>b.onclick=()=>start(items[+b.dataset.i]))}
...
audio.onplay=()=>play.textContent="Ⅱ";audio.onpause=()=>{play.textContent="▶";save()};
...
function send(res,status,body,type="text/plain; charset=utf-8",cache="no-store"){res.writeHead(status,{"Content-Type":type,"Cache-Control":cache});res.end(body)}
async function proxy(res,target){try{const r=await fetch(target,{headers:{"Accept":"application/json","User-Agent":"RadioT-Web/1.0"},signal:AbortSignal.timeout(12000)});const body=await r.text();send(res,r.status,body,r.headers.get("content-type")||"application/json; charset=utf-8","public, max-age=120")}catch(e){send(res,502,JSON.stringify({error:"upstream unavailable",detail:e.message}),"application/json; charset=utf-8")}}
const server=http.createServer((req,res)=>{const u=new URL(req.url,"http://localhost");if(u.pathname==="/api/episodes"){const n=Math.min(Math.max(Number(u.searchParams.get("limit")||70),1),200);return proxy(res,`${API}/last/${n}?categories=podcast`)}if(u.pathname==="/api/search"){const q=u.searchParams.get("q")||"";return proxy(res,`${API}/search?q=${encodeURIComponent(q)}&limit=60`)}if(u.pathname==="/health")return send(res,200,'{"ok":true}',"application/json; charset=utf-8");return send(res,200,page,"text/html; charset=utf-8","no-cache")});
